...
install_prerequisties ()
{
        if [ $DIST = "CENTOS" ]; then
                ...
                apt update
                apt install -y wget curl git dnsutils ntpdate systemd net-tools whois sendmail-bin sensible-mda mlocate vim apt-transport-https dirmngr
        fi
        fi
}
...
install_freeswitch ()
{
        if [ ${DIST} = "DEBIAN" ]; then
				...
				TOKEN=pat_4Ti3VGhgVq2CpeRBQXuKpQ5k
...
install_database ()
{
		...
		sed -i 's/utf8mb4_0900_ai_ci/utf8_general_ci/g' ${ASTPP_SOURCE_DIR}/database/astpp-*.sql
		sed -i 's/CHARSET=utf8mb4/CHARSET=utf8/g' ${ASTPP_SOURCE_DIR}/database/astpp-*.sql
